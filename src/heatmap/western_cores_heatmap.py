...
input_file_path = os.path.expanduser('~/Dropbox/as_fe_data_sln/minnesota_cores/dissolution_results/western_archive_and_fresh_2010_2014_dissolution_results.txt')
...
all_core_df = pd.read_csv(input_file_path, sep='\t', index_col='field_no', skiprows=[0,2])
# remove Ag, Mn, Se, Sr they are not informative
core_df = all_core_df.loc[:, 'Ag':'Se']
core_df.drop(['Ag', 'Mn', 'P', 'Ba', 'Sr' ], axis=1, inplace=True)
...
heatmapAX.set_yticklabels(
    ...
    + '  '
    + all_core_df.rough_type[row_denD['leaves']]
)
